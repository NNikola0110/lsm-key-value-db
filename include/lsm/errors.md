...
| `CorruptionDetected` | Stored bytes fail validation: invalid config or manifest JSON, or the manifest references a missing SSTable (Section 3.9). WAL tail corruption is *not* this — it is handled silently by truncation during recovery, per Section 1. |
